function [Str_sim, Fz_sim, CompData] = Fahrtsimulation10(Str, FZ, const, OptimizationData)
...
% loop um die Streckensimulation durchzuführen
for n = 1:200000
    ...
    if (n == 1)
        ...
        % Zeitoptimierung: maximale Geschwindigkeit zur Zeitgerechten Erreichung des
        % Zieles wird vorberechnet. (kann entfernt werden, wenn die
        % Einhaltung der Fahrtzeit keine Rolle spielt)
        if(maxSpeed(n) > OptimizationData.vOpt)
            % Wenn die zuvor bestimmte max. Geschwindigkeit grösser ist als
            % die für die Zeitoptimierte max. Geschwindigkeit, dann wird
            % diese gewählt, da der Zug ansonsten zu schnell am Ziel
            % angelangt.
            maxSpeed(n) = OptimizationData.vOpt;
        end
        ...
        if (OptimizationData.Bremsstrecke >= x_rest(n))
            STOP = true;
        elseif (OptimizationData.Bremsstrecke < x_rest(n))
            STOP = false;
        end
        ...
        if(STOP == true) 
            ...
            if (OptimizationData.vBrems >= v(n))
    ...
    if(x_rest(n) <= 0.1 && a(n) > -const.accRamp) 
...
xBrake(1:n) = OptimizationData.Bremsstrecke; 
xBrake = xBrake(1:n);
...
% Identität der Strecke wird festgehalten:
%CompData.Zugstrecke(:) = Str.id;

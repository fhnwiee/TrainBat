% function [Str_sim, Fz_sim, CompData] = Fahrtsimulation11(Str, FZ, const, OptimizationData, Fahrtzeit)
...
% erreicht.
% Weitere Verbesserung der Genauigkeit der Simulation:
...
while t < Fahrtzeit
    ...
    for n = 1:200000
        ...
        if (n == 1)
            ...
            v(n) = a(n-1)*const.dt * 3.6 + v(n-1); % (m/s)
            ...
            [BrsStr, vBremsen, Bremszeit, sBremsen] = BerechnungBremsstrecke(FZ, maxSpeed(n), const); % 
            Bremsstrecke(n) = BrsStr;
...
%             if (Bremszeit + t >= Fahrtzeit)
%                 STOP = true;
%             elseif(Bremszeit + t < Fahrtzeit)
%                 STOP = false;
%             end
        ...
        % aktualsierung der zurückgelegten Fahrtzeit:
        t = t + const.dt;

        % Bedingung für das Beenden der Simulation
        if(x_rest(n) <= 1 && a(n) > -const.accRamp && a(n) < const.accRamp)
            % Wenn Destination erreicht ist soll loop verlassen werden.
            break
        end

    end
    % Die Geschwindigkeitsvorgaben werden reduziert, da die Strecke
    % ansonsten zu schnell zurückgelegt wird:
    Str.speedLimits(:,2) = Str.speedLimits(:,2) - 0.05;
end
...
CompData.time = seconds(transpose(0.1:0.1:(length(Str_sim.Strecke)/10)));

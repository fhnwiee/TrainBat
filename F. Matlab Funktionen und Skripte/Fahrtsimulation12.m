function [Str_sim, Fz_sim, CompData] = Fahrtsimulation12(Str, FZ, const)
% Diese Funktion simuliert eine Strecke einer Zugfahrt schrittweise. In
% jedem Zeitschritt werden die Parameter neu berechnet.
...
% Die Strecke wird mit der maximalen Geschwindigkeit so schnell wie möglich
% zurückgelegt.
...
for n = 1:200000
    ...
    if (n == 1)
        ...
        % Abspeichern der aktuellen Geschwindigkeit des Zeitschritts.
        ...
        %vBeschl(n) = OptimizationData.vOpt;
        vBeschl(n) = maxSpeed(n);
        if(v(n) <= maxSpeed(n))
            for i = 0:const.accRamp:a(n-1)
                vBeschl(n) = vBeschl(n) - i * const.dt * 3.6;
            end

        end

        % Berechnung der aktuellen Bremsstrecke:
        if (STOP == false)
            [BrmsStr, ~, vBrems] = CalcBremsstreckeV3(v(n), const, FZ, a(n-1));
        end
        Bremsstrecke(n) = BrmsStr;
        ...
        if(STOP == true)
            ...
            % werden.
        elseif (v(n) >= vBeschl(n) && a(n-1) > 0 && v(n) <= maxSpeed(n))
        ...
        elseif (STOP == false)
            ...
            elseif ((F_A(n) - (W_run(n) + W_st(n) + W_curve(n))) > 0 && F_wheel(n) >= (W_run(n) + W_st(n) + W_curve(n)))
                ...
                elseif (v(n) > maxSpeed(n) * 1.01)
                    F_brake(n) = -FZ.mu_brake * G_T * const.g * cos(s(n)/1000); % (kN)
                    if (a(n-1) - const.accRamp > FZ.maxDeceleration && a(n-1) - const.accRamp >= F_brake(n)/FZ.mass + const.g * sin(s(n)/1000))
                        a(n) = a(n-1) - const.accRamp;
                    elseif(a(n-1) - const.accRamp > FZ.maxDeceleration && a(n-1) - const.accRamp < F_brake(n)/FZ.mass + const.g * sin(s(n)/1000))
                        a(n) = F_brake(n)/FZ.mass + const.g * sin(s(n)/1000);
                    elseif(a(n-1) - const.accRamp <= FZ.maxDeceleration && FZ.maxDeceleration >= F_brake(n)/FZ.mass + const.g * sin(s(n)/1000))
        ...
        elseif (W_tot(n) * v(n)/3.6 < 0 && v(n) > FZ.minSpeedRegBraking)
            if(abs((W_tot(n) * v(n)/3.6 + const.AuxP) * const.nPantograph) <= FZ.maxRegPower)
                ...
                P_r(n) = -FZ.maxRegPower + const.AuxP/const.nPantograph; % (kW)
            end
        else
...
% [x, a, v, x_brake, x_rest, s, F_brake, F_A, W_run, W_st, W_a, W_tot, mu, F_wheel, P_r, E, A]
